...
<!--<img src="{{asset('images/mtn.jpg')}}" alt="" width="100%">-->
<!-- Main -->
<article id="main">
  <section class="data-top-header wrapper style3 special container 100%">
    ...
    <ol class="data-breadcrumbs arrows center" >
      ...
      <li><a href="{{url('/data/mtn')}}" style="color:#4169E1">MTN</a></li>
    </ol>
  </section>
  ...
  <section class="wrapper style3 container special 150%">
    <div class="row">
      <div class="3u 12u(narrower) 12u(mobile)">
        <img class="data-header-img" src="{{asset('img/mtn1.png')}}" width="200px">
      </div>

      <div class="9u 12u(narrower) data-intro">
        <h1>Take advantage of MTN Nigeria's wide coverage and ever reliable connection today. We have gone the extra mile to bring you MTN Data bundles at the best prices ever!</h1>
      </div>
    </div>

    <header class="major">
      <h2><strong>MTN&nbsp;</strong>Data&nbsp;<strong>Options</strong></h2>
    </header>
            ...
            @if ($k->approval == NULL)
              <div>
                @if ($k->webspec == 3)
                  ...
                  <span>&mdash; <strong>{{$k->name}}</strong>, {{$k->subject}} <span class="data-badge">DATA</span></span>
                @elseif ($k->webspec == 2)
                  <p class="testimonial-body"> <i class="icon fa-quote-left"></i> {{$k->message}} &nbsp;<i class="icon fa-quote-right"></i></p>
                  <span>&mdash; <strong>{{$k->name}}</strong>, {{$k->subject}} <span class="web-badge">WEB</span></span> 
                @elseif ($k->webspec == 1)
                  <p class="testimonial-body"> <i class="icon fa-quote-left"></i> {{$k->message}} &nbsp;<i class="icon fa-quote-right"></i></p>
                  <span>&mdash; <strong>{{$k->name}}</strong>, {{$k->subject}} <span class="brand-badge">BRAND</span></span>
                @endif
              </div>
            @endif
          @endforeach
  ...
  <section class="wrapper style4 container special 150%">
    ...
    <br>
    <div id="ss_menu" style="text-align: left">
      ...
      <div class="ss_content">Yes. Please dial *131*4# or SMS 2 to 131 to check your MTN data bundle balance.</div><br>
      <div class="ss_button">CAN I SHARE MY DATA WITH MY FRIENDS?</div>
      <div class="ss_content">Unfortunately, you cannot share your MTN data bundles with friends at this time. Please note that this restriction is from the network.</div>
      <br>
      <div class="ss_button">WILL THIS DATA PLAN WORK WITH MY DEVICE?</div>
      <div class="ss_content">Our MTN Data Plans work perfectly with all internet enabled devices. But please note that functionality may be restricted on older Blackberry devices.</div>
      <br>
      <div class="ss_button">HOW LONG DOES THE DATA LAST?</div>
      <div class="ss_content">All our MTN Data Bundles are valid for 30 days, but depending on your usage, it may be exhausted before that time.</div>
      <br>
      <div class="ss_button">CAN MY DATA BUNDLE BE ROLLED-OVER OR TOPPED-UP?</div>
      <div class="ss_content">If you purchase a new data bundle before the expiry of the current one, all your previous data will be rolled over.</div>
      <br>
      <div class="ss_button">HOW LONG WILL IT TAKE TO GET THE DATA?</div>
      <div class="ss_content">We send out data bundles instantly as soon as we confirm payment.</div>
      <br>
      <div class="ss_button">HOW CAN I WORK WITH YOU AND RESELL YOUR DATA TO MY CUSTOMERS?</div>
      <div class="ss_content">Please contact us on 07033044456 or 09097619649 for more info.</div>
      <br>
      <div class="ss_button">WHY SHOULD I TRUST YOU (MIGHTY DATA)?</div>
      <div class="ss_content">We are a fully fuctional business, registered with the Corporate Affairs Commission (CAC). We use only our corporate accounts for collection of payment. Also, we have many customers who have been enjoying our services for quite a while.</div>
      <br>
    </div>
  </section>
</article>
